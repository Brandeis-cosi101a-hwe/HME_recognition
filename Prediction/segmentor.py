...
class Segmentor(object):
    ...
    def cropImg(im_temp2, new_rect):
        ...
        return (img, [rect[0], rect[1], rect[0] + rect[2], rect[1] + rect[3]])
        # cv2.rectangle(im, (rect[0], rect[1]), (rect[0] +
        #                                        rect[2], rect[1] + rect[3]), (0, 255, 0), 3)

    @staticmethod
    def process(image_path):
        result_imgs, result_rects = []
                ...
                for j in range(i, len(bars_rect) - 1):
                    ...
                    if overlap(bars_rect, i, j):
                        ...
                        new_rect = (min(bars_rect[i][0], bars_rect[j][0]),
                                    ...
                                    max(bars_rect[i][0] + bars_rect[i][2], bars_rect[j][0] +
                                        bars_rect[j][2]) - min(bars_rect[i][0], bars_rect[j][0]),
        ...
        for i in range(0, len(bars_rect)):
            rect = bars_rect[i]
            im_temp2 = im2[rect[1]:rect[1] +
                           rect[3], rect[0]:rect[0] + rect[2]]
            blobs_labels = measure.label(
                im_temp2, connectivity=5, neighbors=8, background=0, return_num=False)
            im_temp2 = (blobs_labels % 255 != 0) * 1
            cropped_img, cropped_rect = cropImg(im_temp2, rect)
            result_imgs.append(cropped_img)
            result_rects.append(cropped_rect)
            # cv2.rectangle(
            #     im, (rect[0], rect[1]), (rect[0] + rect[2], rect[1] + rect[3]), (0, 255, 0), 3)
        return (result_imgs, result_rects)
