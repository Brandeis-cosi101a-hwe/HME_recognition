...
def img_combine(img1, img2, r1, r2, base_rect):
    ...
    img1 = skimage.util.pad(img1, ((
        r1[1], h - r1[3] - r1[1]), (r1[0], w - r1[2] - r1[0])), 'constant', constant_values=0)
    img2 = skimage.util.pad(img2, ((
        ...
        for i in range(-1, len(bars_rect) - 1):
            ...
            for j in range(i, len(bars_rect) - 1):
                ...
                if overlap(bars_rect, i, j):
                    ...
                    new_im = img_combine(
                        bars_im[i], bars_im[j], bars_rect[i], bars_rect[j], new_rect)
                    del bars_rect[j]
                    del bars_rect[i]
                    del bars_im[j]
                    del bars_im[i]
